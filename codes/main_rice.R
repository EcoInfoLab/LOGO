...
xls_path <- file.path(OD,"data","DSSAT.xlsx")
...
#source("GenotypeChange.r")
#source("EcotypeChange.r")
#source("summary_daily_values.R")
#source("PIDAT.r")
#source("DailyOUT.r")
#source("makeBatch.R")
#source("makeTemplate.R")
...
model_options <- list(CultivarID=CultivarID, GenotypeFileName=GenotypeFileName, CropName=CropName,
                      OD=OD, GD=GD, DSSATD=DSSATD, xfilename=xfilename)
...
out_dir <- file.path(OD,"results",test_case,variety)
...
#}
suffix <- paste0("_",variety)
obs_data_folder <- "data"
obs_data_path <- file.path(OD,obs_data_folder,paste0("cal_4_obs_",test_case,suffix,".txt"))
obs_unit_path <- file.path(OD,obs_data_folder,paste0("cal_4_obs_",test_case,"_units.csv"))
## Get the reference date for each situation
template_path <- file.path(OD,"data",paste0("simulations_template.txt"))
makeTemplate(obs_data_path,template_path)
...
if (is.null(res_it2)) {
  ...
  weight_it2[which(weight_it2==0)] <- .Machine$double.xmin #sinu, 15-Mar-2025
