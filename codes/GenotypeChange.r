...
GenotypeChange<-function(GD, DSSATD, OD, CropName, GenotypeFileName, CultivarID, RunNumber, RandomMatrix)
{
...
LineNumber<-grep(pattern=paste0("^",CultivarID), GenotypeFile); #Get the number of the line where the cultivar "GLUECUL" is located.
HeaderLineNumber<-grep(pattern="^@VAR", GenotypeFile);
OldLine<-GenotypeFile[LineNumber];#Get the line according to the line number.
HeaderLine<-GenotypeFile[HeaderLineNumber]
